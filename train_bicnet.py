...
from guessing_sum_env import GuessingSumEnv
...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, batch_size):
        ...
        self.action_gradient = tf.placeholder(
            tf.float32, (NUM_AGENTS, None, NUM_AGENTS, OUTPUT_LEN), name="action_gradient"
        )
        ...
        with tf.name_scope("actor_gradients"):
            ...
            for i in range(NUM_AGENTS):
                for j in range(NUM_AGENTS):
                    grads.append(
                        tf.gradients(
                            self.out[:, j], self.network_params, -self.action_gradient[j][:, i]
                        )
                    )
            grads = np.array(grads)
            self.unnormalized_actor_gradients = [
                tf.reduce_sum(list(grads[:, i]), axis=0) for i in range(len(self.network_params))
            ]
        ...
        self.optimize = self.optimize.apply_gradients(
    ...
    def create_actor_network(self, name):
        inputs = tf.placeholder(
            tf.float32, shape=(None, NUM_AGENTS, VECTOR_OBS_LEN), name="actor_inputs"
        )
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, gamma, num_actor_vars):
        ...
        self.predicted_q_value = tf.placeholder(
            tf.float32, (None, NUM_AGENTS, 1), name="predicted_q_value"
        )
        ...
        self.loss = tf.squeeze(
            1.0 / M * tf.
            reduce_sum(tf.reduce_sum(tf.square(self.predicted_q_value - self.out), axis=1), axis=0),
            name="critic_loss"
        )
    ...
    def create_critic_network(self, name):
        inputs = tf.placeholder(
            tf.float32, shape=(None, NUM_AGENTS, VECTOR_OBS_LEN), name="critic_inputs"
        )
        action = tf.placeholder(
            tf.float32, shape=(None, NUM_AGENTS, OUTPUT_LEN), name="critic_action"
        )
    ...
    def train(self, inputs, action, predicted_q_value):
        return self.sess.run(
            [self.out, self.optimize, self.loss],
            feed_dict={
                self.inputs: inputs,
                self.action: action,
...
def train(sess, env, args, actor, critic):
    ...
    writer = tf.summary.FileWriter(
        args['summary_dir'] + " actor_lr" + str(args['actor_lr']) + " critic_lr" +
        str(args["critic_lr"]), sess.graph
    )
        ...
        for j in range(int(args['max_episode_len'])):
            ...
            if replay_buffer.size() > int(args['minibatch_size']):
                ...
                predicted_q_value, _, loss = critic.train(
                    s_batch, a_batch,
                    np.reshape(r_batch, (int(args['minibatch_size']), NUM_AGENTS, 1))
                )
                ...
                print(
                    '| Reward: {:.4f} | Episode: {:d} | Qmax: {:.4f}'.format(
                        np.mean(r_batch), i, (ep_ave_max_q / float(j + 1))
                    )
                )
...
def main(args=None):
    ...
    with tf.Session(config=config) as sess:
        ...
        actor = ActorNetwork(
            sess, state_dim, action_dim, float(args['actor_lr']), float(args['tau']),
            int(args['minibatch_size'])
        )

        critic = CriticNetwork(
            sess, state_dim, action_dim, float(args['critic_lr']), float(args['tau']),
...
def parse_arg(args=None):
    ...
    parser.add_argument(
        '--minibatch-size', help='size of minibatch for minibatch-SGD', default=1024
    )

    # run parameters
    parser.add_argument('--random-seed', help='random seed for repeatability', default=1234)
    parser.add_argument(
        '--max-episodes', help='max num of episodes to do while training', default=9999999999999
    )
