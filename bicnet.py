...
class BiCNet:
    ...
    @staticmethod
    def base_build_network(obs):
        encoded = BiCNet.shared_dense_layer("encoder", obs, HIDDEN_VECTOR_LEN)

        hidden_agents = tf.unstack(encoded, NUM_AGENTS, 1)

        lstm_fw_cell = tf.nn.rnn_cell.BasicLSTMCell(
            HIDDEN_VECTOR_LEN, forget_bias=1.0, name="lstm_fw_cell"
        )
        lstm_bw_cell = tf.nn.rnn_cell.BasicLSTMCell(
            HIDDEN_VECTOR_LEN, forget_bias=1.0, name="lstm_bw_cell"
        )
        outputs, _, _ = tf.nn.static_bidirectional_rnn(
            lstm_fw_cell, lstm_bw_cell, hidden_agents, dtype=tf.float32
        )
    ...
    @staticmethod
    def actor_build_network(name, obs):
        with tf.variable_scope(name):
            outputs = BiCNet.base_build_network(obs)
            return BiCNet.shared_dense_layer("output_layer", outputs, OUTPUT_LEN)

    @staticmethod
    def shared_dense_layer(name, obs, output_len):
        H = []
    ...
    @staticmethod
    def critic_build_network(name, obs, action):
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            outputs = BiCNet.base_build_network(tf.concat([obs, action], 2))
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        observation = tf.placeholder(
            tf.float32, shape=(None, NUM_AGENTS, VECTOR_OBS_LEN), name="observation"
        )
        ...
        print(
            ...
            (BATCH_SIZE, NUM_AGENTS, OUTPUT_LEN), "== (BATCH_SIZE, NUM_AGENTS, OUTPUT_LEN)"
        )
        ...
        print(
            sess.run(critic_out, feed_dict=feed_dict).shape, "==", (BATCH_SIZE, NUM_AGENTS, 1),
            "== (BATCH_SIZE, NUM_AGENTS, 1)"
        )

        feed_dict = {observation: np.random.random_sample((1, NUM_AGENTS, VECTOR_OBS_LEN))}
        print(
            sess.run(actor_out, feed_dict=feed_dict).shape, "==", (1, NUM_AGENTS, OUTPUT_LEN),
        ...
        print(
            sess.run(critic_out, feed_dict=feed_dict).shape, "==", (1, NUM_AGENTS, 1),
            "== (BATCH_SIZE, NUM_AGENTS, 1)"
        )
